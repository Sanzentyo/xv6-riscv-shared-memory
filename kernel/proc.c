...
found:
  p->pid = allocpid();
  p->state = USED;
  p->shmem_access_count = 0;
  for(int i=0; i < SHMEM; i++){//.ADD
    p->shmem_access[i]=false;
    p->shmem_access_count--;
  }
...
void* get_shmem_access (int page_number, int pid){ //.ADD
  ...
  VA=VASHMEM(p->shmem_access_count);
...
void safe_physical_address (void * phaddr, int i){
  ...
  shmem[i].state=NOT_SET; // 最初なのでNOT_SET
  char name[13] = "shmem_state_";
  name[12] = (char)(i + '0');
  initlock(&shmem[i].state_lock, name);
  shmem[i].using_top_pid = -1;
}
